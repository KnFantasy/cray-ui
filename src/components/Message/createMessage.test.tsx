...
describe('createMessage', () => {
  test('调用方法应该创建对应的 Message 组件', async () => {
    ...
    expect(document.querySelector('.cray-message')).toBeTruthy()
    instance.destory()
    await rAF()
    console.log(document.body.innerHTML)
    expect(document.querySelector('.cray-message')).toBeFalsy()
  })
  ...
  test('多次调用方法应该创建多个实例', async () => {
    ...
    await rAF()
    const elements = document.querySelectorAll('.cray-message')
  ...
  test('创建多个实例应该设置正确的 offset', async () => {
    ...
    const elements = document.querySelectorAll('.cray-message')
    expect(elements.length).toBe(2)
    // 在JS-dom中无法使用getBoundingClientRect
